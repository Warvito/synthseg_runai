...
while IFS=$'\t' read -r -a myArray; do
  if (($COUNTER >= ${minimum} && $COUNTER < ${maximum})); then
    ...
    OUTPUT_VOL="/target/${myArray[1]}_${myArray[2]}_T1w_sample/${myArray[1]}_${myArray[2]}_T1w_vol.csv"
...
done < "/project/outputs/ids/existing_samples_subset_preprocessed_HAGAN.tsv"
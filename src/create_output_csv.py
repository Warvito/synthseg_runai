...
ids_df = pd.read_csv(
    "/project/outputs/ids/existing_samples_subset_preprocessed_CCE.tsv",
    sep="\t"
)
volumes_list = []
missing_vol = []
...
missing_volumes_df.to_csv(
    "/project/outputs/missing_preprocessed_cce.tsv",
    sep="\t",
    index=False
)
volumes_df = pd.DataFrame(volumes_list)
volumes_df.to_csv(
    "/project/outputs/volumes_synthseg_ccegan.tsv",
    sep="\t",
    index=False
)